...
{
    public class UtilsForClientYamlFilesGenerator : IClientYamlFileGenerator
    {
        private const string FileNameSeperator = "_";

        private readonly ILogger<UtilsForClientYamlFilesGenerator> _logger;
        ...
        private readonly IClientGuidIndexService _indexService;
        private readonly IClientAssistRuleIndexService _assistRuleIndexService;
        ...
        public UtilsForClientYamlFilesGenerator(
            ILogger<UtilsForClientYamlFilesGenerator> logger,
            ...
            IClientGuidIndexService indexService,
            IClientAssistRuleIndexService assistRuleIndexService,
        ...
        public async Task<(string path, int fileCount)> GenerateYamlFilesForResourceTypeValues(string path, List<Guid> libraryIds)
        {
            ...
            foreach (var v in list)
            {
                ...
                var componentId = await _indexService.GetIntAsyncWithoutThrowingErrorAsync(v.ComponentGuid);
                if (componentId == 0)
                {
