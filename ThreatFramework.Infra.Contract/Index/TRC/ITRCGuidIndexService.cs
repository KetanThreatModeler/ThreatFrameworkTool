...
namespace ThreatModeler.TF.Infra.Contract.Index.TRC
{
    public interface ITRCGuidIndexService
    {
        Task GenerateAsync();
        Task RefreshAsync();
        Task GenerateForLibraryAsync(IEnumerable<Guid> libIds);
        Task<int> GetIntAsync(Guid guid);
        Task<int> GetIntForClientIndexGenerationAsync(Guid guid);
        Task<(int entityId, int libId)> GetIntIdOfEntityAndLibIdByGuidAsync(Guid guid);
        Task<Guid> GetGuidAsync(int id);
        Task<Guid> GetGuidWithoutThrowingError(int id);
